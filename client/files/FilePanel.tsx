/**
 * F7 本地文件面板 —— 广播栏下方的收起条；展开后：工具栏（上一级 / 换目录 / 刷新）+ 面包屑 + 列表。
 * 单击选中、双击目录进入、双击文件交给 onOpenFile（编辑器）。远端面板（S5）复用 FileList。
...
export function FileList(props: {
  ...
  const { entries, selected, loading, error, onSelect, onOpenDir, onOpenFile } = props
      ...
      {entries.map(e => {
        ...
        return (
          <div
            ...
            title={e.path}
          >
            <span className="ico">{isDir ? '📁' : e.kind === 'symlink' ? '🔗' : '📄'}</span>
      ...
      {s.expanded && (
        <div className="tm-fpBody">
          <div className="tm-fpToolbar">
            <button className="tm-tbtn" disabled={atRoot} onClick={() => void store.up()} title="上一级"><IconChevronUpOutline14 /> 上一级</button>
            <button className="tm-tbtn" onClick={() => setPicking(true)} title="切换本地根目录"><IconFolderOpen16 /> 换目录</button>
            <button className="tm-tbtn" onClick={() => { void store.refresh(); toast('本地列表已刷新') }} title="刷新"><IconRefreshOutline14 /> 刷新</button>
            <span className="hint">双击文件 → 编辑器打开</span>
          </div>
          <div className="tm-fpPath">
            {crumbs.map((c, i) => (
              <span key={c.path}>
                {i > 0 && <span>/</span>}
                <span className={`seg ${i === crumbs.length - 1 ? 'cur' : ''}`} onClick={() => void store.enter(c.path)}>{c.label}</span>
              </span>
            ))}
          </div>
          <FileList
