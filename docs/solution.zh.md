...
> **串口已拍板（2026-08-26）**：不进 MVP，作为**交付后第一个扩展项**紧跟。接缝已留：新增 `SerialTransport` 实现传输接口 + 表单加串口参数即可，不动主干。
...
| ~~串口是否进 MVP~~ | ✅ 已拍板（2026-08-26） | — | **不进 MVP，作为交付后第一个扩展项紧跟**；接缝已留（`SerialTransport` + 表单串口参数） |
