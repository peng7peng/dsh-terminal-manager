...
MVP必需满足的需求：
1.	基于芯片验证团队实际使用场景，在deepseek harness的基础上开发插件，参考 AgentTerm 实现SSH和Telnet连接，通过Agent发送指令和回传日志操作，同时Terminal面板展示明细信息，提升调试效率
2.	需要实现多端口同时支持能力，Terminal面板也能多端口展示
3.	初版MaaS服务绑定灵枢平台，API 简化配置
...
> 批注回应（AgentTerm 参考）：设计连接表单与交互前，先看 AgentTerm 的 SSH/Telnet 连接设计，沿用用户已习惯的操作方式——见文末待确认项（需要 AgentTerm 的链接/截图/安装包）。
...
> 批注回应（M4 界面三条原则）：① 终端展现参考 **iTerm** 的观感；② **断开按钮收敛**——状态条芯片保留 ✕，终端标题条与连接卡片不再重复放；③ **连接与终端分离**——两个页签各司其职，不同屏叠加。
...
|---|---|---|
| S4 | 手动敲命令 | 按键即达、输出即显 |
| S5 | 广播到全部会话 | 所有打开的终端同时回显，慢设备不拖慢别人；回显样式与正常终端显示保持一致 |
| S6 | 广播到部分会话 | 芯片点选目标，按钮显示目标数 |

> 批注回应：广播的回显样式与正常终端显示**保持一致**（已写入 S5 期望结果）。
...
串口、文件传输（SFTP）、设备日志文件拉取、多用户协作、云端部署、审计录屏、**SSH 跳板机/堡垒机（ProxyJump）**——跳板机若你的环境必需，联调前告诉我，实现上预留了接缝（见 3.8 扩展表）。

> ⚠️ **串口待拍板**：产品负责人的实际场景含**芯片串口**（2026-08-26 确认），与本边界冲突。若串口进 MVP：新增 `SerialTransport`（传输接口已为它留好接缝）+ 连接表单加串口参数（波特率/数据位等）。
...
| Telnet 实现 | Node 自带 `net`（裸 TCP），**已定案** | 目标设备以网络设备/ESL 环境为主，裸 TCP 足够；协议协商接缝保留，个别设备异常可按设备切换 | `telnet-client` 库（年久失修，暂不需要） |
...
> 批注回应：**优先级**与**高危操作**已在 3.7 定义（优先级规则 + B8 命令守卫）。
...
#### B8 命令守卫 — AI 发送前的安检门（详见 3.7）

```mermaid
flowchart LR
    B6["B6：AI 要发命令"] --> M["B8 命令守卫（纯逻辑）<br/>黑名单匹配 + 白名单豁免"]
    M -- "放行" --> B4["B4：执行"]
    M -- "拦截" --> R["COMMAND_BLOCKED + 日志"]
```

**① 这个模块干什么**：检查 AI 发起的命令文本是否命中危险命令黑名单（可配置、可按连接加白名单），命中即拦截并留日志；人的键盘输入不经过它。
**② 谁调用它**：B6 工具层（`tm_send` / `tm_send_all` 发送前）。
**③ 它依赖谁**：什么都不依赖——纯逻辑，规则从插件配置读取。
...
| `COMMAND_BLOCKED` | 命令命中黑名单（命令守卫拦截，见 3.7） |
| `PROTO_ERROR` | 协议层错误（SSH 通道建立失败等） |

## 3.7 命令安全防护（AI 发危险命令怎么办）

**结论**：DSH 能拦「工具调用」但看不见「命令内容」，所以两道防线叠加：

**第一道防线（DSH 自带，用户配置）**：DSH 的工具执行策略可对六个 `tm_*` 工具设置「允许 / 禁止 / 每次先问我」——适合"压根不让 AI 碰终端"或"AI 每次动手都要我点头"的场合。这是部署配置，不改我们代码。

**第二道防线（插件内建，新增模块 B8 命令守卫）**：内容级检查——

- **只检查 AI 发起的发送**（`tm_send` / `tm_send_all`）。人自己在终端敲的字**不拦**：那是字符级的（有退格、拼接、行编辑），没法可靠拦截，而且人自己对自己负责。
- **规则 = 可配置的黑名单**，默认含典型破坏性命令模式：`rm -rf /`、`mkfs`、`dd if=…of=/dev/…`、`shutdown`、`reboot`、`format`、`erase`（网络设备）等；支持按连接加白名单（同一命令在某台设备上可能是正常操作）。
- **处置**：命中黑名单 → 拒绝执行，返回错误码 `COMMAND_BLOCKED`，AI 会看到拒绝原因并告知你；「要确认后再放行」的场景交给第一道防线的"先问我"，MVP 不重复造确认弹窗。
- **广播时逐台检查**：命令对某个目标命中黑名单，该台拒绝并单独标记，不影响其他台。
- **实现位置**：纯逻辑模块（与 B5 同类，无 I/O，好测试），由 B6 工具层在发送前调用；所有拦截写日志。

**B4 的优先级规则（回答图下批注）**：键盘输入永远实时、不排队；命令发送每会话独占——已有一条在跑时新的发送直接返回 `SESSION_BUSY`；广播逐台并行、互不阻塞。

## 3.8 可扩展性设计（为后续功能留的口子）
...
| 项 | 问题 | 影响 | 处理 |
|---|---|---|---|
| ~~Telnet 设备类型~~ | ✅ 已关闭（2026-08-26） | — | 用户确认：Telnet 主要连网络设备（含 ESL 环境）→ **定案：Node `net` 裸 TCP**，协议协商接缝保留 |
| 设备提示符形态 | 基本明确为网络设备提示符 | M2 完成判定 | **不再阻塞**：预置常见网络设备提示符模板（`[>#\]]\s*$` 一类），连接表单按连接选用；不启用时用静默期判定兜底 |
| 连接规模 | ✅ 已确认 | — | 通常 **1–3 个并发会话**，现有设计全部够用 |
| 串口是否进 MVP | 用户实际场景含芯片串口，但 MVP 边界目前排除串口 | 若进：新增 `SerialTransport` + 表单串口参数（波特率等） | **待用户拍板** |
| 跳板机环境 | 连内网设备要不要过堡垒机/跳板机？ | 若需要：B2 加串接连接支持 + 表单加字段 | **用户暂不确定**。MVP 先不做；联调卡在这里就启动扩展项 |
| 灵枢平台 MaaS 绑定 | 1.1 第 3 条「初版 MaaS 服务绑定灵枢平台，API 简化配置」具体指什么？ | 影响配置模块与文档 | **用户挂起，不阻塞** |
| AgentTerm 参考 | 需要 AgentTerm 的设计参考（用户使用习惯） | M4 连接表单与交互细节 | **用户后续口头描述**；拿到后在 M4 开工前对照调整 |
