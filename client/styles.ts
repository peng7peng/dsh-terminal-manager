...
/* 终端列 + 连接面板紧贴相连（无留白）。终端列宽度由 JS 按视口算出固定值（~600），聊天区自动加宽填满左侧 */
...
.tm-reconnect-small:hover { background:var(--dsw-alias-state-info-primary, #2563eb); }
...
.tm-sideHead .t { font-size:15px; font-weight:600; }
...
.tm-ptab { flex:1; text-align:center; padding:6px 0; font-size:13px; font-weight:600; color:var(--dsw-alias-label-tertiary, #888); cursor:pointer; border:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); border-radius:7px; user-select:none; }
...
.tm-fld label { font-size:13px; color:var(--dsw-alias-label-secondary, #666); flex:0 0 auto; min-width:42px; text-align:right; }
.tm-fld input { background:var(--dsw-alias-bg-base, #fff); border:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); border-radius:7px; color:var(--dsw-alias-label-primary, #000); padding:8px 10px; font-size:14px; outline:none; width:100%; flex:1; box-sizing:border-box; }
...
.tm-fld select { background:var(--dsw-alias-bg-base, #fff); border:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); border-radius:7px; color:var(--dsw-alias-label-primary, #000); padding:8px 10px; font-size:14px; outline:none; flex:1; box-sizing:border-box; }
.tm-req { color:var(--dsw-alias-state-error-primary, #ef4444); }
.tm-opt { font-size:11px; color:var(--dsw-alias-label-caption, #aaa); border:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); border-radius:5px; padding:0 4px; margin-left:3px; }
.tm-pwdWrap { position:relative; flex:1; }
.tm-pwdWrap input { padding-right:28px; }
.tm-pwdToggle { position:absolute; right:8px; top:50%; transform:translateY(-50%); cursor:pointer; font-size:15px; opacity:.6; user-select:none; }
.tm-pwdToggle:hover { opacity:1; }
.tm-advToggle { display:flex; align-items:center; gap:5px; cursor:pointer; font-size:12px; color:var(--dsw-alias-label-tertiary, #888); margin:8px 0 4px; user-select:none; }
...
.tm-ctxItem { padding:6px 16px; cursor:pointer; font-size:13px; color:var(--dsw-alias-label-secondary, #666); }
...
.tm-authSwitch span { flex:1; text-align:center; font-size:12px; padding:5px 0; border:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); border-radius:6px; cursor:pointer; color:var(--dsw-alias-label-secondary, #666); user-select:none; }
...
.tm-btn { border:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); background:var(--dsw-alias-bg-base, #fff); color:var(--dsw-alias-label-secondary, #666); border-radius:8px; padding:8px 12px; font-size:13px; cursor:pointer; }
...
.tm-secLabel { font-size:12px; color:var(--dsw-alias-label-caption, #aaa); font-weight:600; letter-spacing:.5px; margin-bottom:4px; }
.tm-ritem { display:flex; align-items:center; gap:7px; padding:8px 10px; border-radius:7px; background:var(--dsw-alias-bg-base, #fff); border:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); font-size:13px; cursor:pointer; margin-bottom:4px; }
.tm-ritem:hover { border-color:var(--dsw-alias-border-l3, rgba(0,0,0,.16)); }
.tm-pico { font-size:11px; font-weight:700; padding:2px 6px; border-radius:4px; font-family:var(--dsw-font-code, monospace); }
...
.tm-sub { display:block; font-size:12px; font-weight:400; color:var(--dsw-alias-label-caption, #aaa); font-family:var(--dsw-font-code, monospace); }
...
.tm-term-closed { opacity:0.6; }
.tm-term-closed .tm-paneBody { pointer-events:none; }
.tm-term-maximized { grid-column: 1 / -1; grid-row: 1 / -1; }
