...
- **会话共享**：把已打开的 SSH/Telnet 会话通过 TCP 共享给其他客户端，实时收发终端数据；
- **日志**：插件运行日志按 5 MiB、最多 5 个文件轮转；会话设备输出日志可按会话启停；
- 纯本地运行，不依赖 MCP。
...
pnpm test             # 232 项 vitest（模拟设备和本地 echo，不碰真设备）
```
...
- **会话共享没有认证且完全可写**：任何能访问共享监听端口的客户端都能查看输出并控制终端；只在可信网络中手动开启。
- **会话日志不会轮转或自动删除**：默认关闭；开启后只直接记录设备输出，文件会持续增长到会话关闭或用户停止日志，请关注磁盘空间。
- **无远端文件传输**（SFTP 上传 / 下载、远端文件面板在后续迭代）、无审计录屏、无多用户协作。
