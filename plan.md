...
| `client/*.module.css` | CSS Modules 样式，随 DSH 明暗主题 |
...
2. `remotes.ts`：`termManager` 控制面（浏览器半 M3 会消费，先落地便于联调）。
3. `tools.spec.ts`：经测试上下文注册与调用，断言 schema、返回、取消（`exec.signal`）、错误形态。
4. 验收：在真实 `dsh` 会话里让 Agent 调 `tm_connect/tm_send`（对 M1 的进程内测试服务器），输出完整回传。

**M3 — 数据面 + GUI**
1. `ws-io.ts`：`/term-io` upgrade 路由 + 帧协议 + 信任栅栏；`attach` 回放缓冲尾部；单测走内存 socket。
2. `client/ws.ts` + `TermView.tsx`：单会话渲染、键入回传、`resize`。
3. `TerminalPanel/ConnectionsTab/TerminalsTab`：双页签、连接表单、状态徽标、广播栏。
4. 真实启动截图验证三个画面：连接页配置两条连接；终端页双会话实时输出；广播后双终端回显。
5. 验收：截图比对；`details` 栏收起/展开、切换聊天会话后重新展开，会话与输出无丢失。
...
| `ssh2` 在 Windows 上的可选原生加密依赖安装失败 | 用其纯 JS 回退路径；CI/本地均验证一次安装 |
| 框架开发者预览破坏性变更 | 依赖用 `link:` 锁相邻检出；每个里程碑结束跑一遍真实启动冒烟 |
| `@xterm/*` 进客户端 bundle 的兼容性 | M3 第一步先做最小渲染冒烟，再铺组件 |
| 完成判定对慢设备误判 | 参数按连接可调；`waitReason` 暴露给 Agent；M4 联调真实设备微调 |
...
- 截图证据（M3/M4）：连接页、双终端页、广播回显、Agent 调用回传画面。
...
- **单一实现主线**（M0→M4 强依赖链），不开多会话改代码，避免冲突与评审超载。
- 命名子代理仅用于验证：`verifier`（跑构建/测试/启动截图，报告证据）在 M0/M3/M4 各用一次；每次派发说明原因，报告界限明确（跑了什么、看到什么、没查什么）。
- 若 M3 中 UI 样式打磨与 WS 联调被证明互不干扰，再评估是否拆第二个会话；默认不拆。
