...
describe('插件 Config schema', () => {
  it('schema 校验空对象填默认值', () => {
    const value = Config({})
    expect(value).toEqual({ workspaceRoot: '' })
  })

  it('schema 保留用户给的值', () => {
    const value = Config({ workspaceRoot: 'D:/dut' })
    expect(value).toEqual({ workspaceRoot: 'D:/dut' })
  })
  ...
  it('resolveConfig：空 / 空白 / 缺省 workspaceRoot 落到 cwd；显式值 trim 后保留', () => {
    const cwd = () => 'D:/cwd'
    expect(resolveConfig(undefined, cwd)).toEqual({ workspaceRoot: 'D:/cwd' })
