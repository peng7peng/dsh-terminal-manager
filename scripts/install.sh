...
if ! command -v dsh &>/dev/null; then
    err "未找到 dsh 命令"
    echo "请先安装 DeepSeek Harness："
    echo "  npm install -g @deepseek-ai/dsh"
    echo "或从源码安装：https://github.com/deepseek-ai/deepseek-harness"
    exit 1
fi
ok "dsh $(dsh --version 2>/dev/null || echo '')"
...
# 6. 注册到 DSH profile
info "注册到 DSH profile: $PROFILE"
dsh plugin --profile "$PROFILE" add "dsh-terminal-manager@link:$PLUGIN_DIR"
ok "安装完成"
