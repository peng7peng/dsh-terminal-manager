...
- ✅ 插件入口装配测试（10 项）
- ✅ HTTP 路由层测试（8 项）
- ✅ WebSocket 数据流通道生命周期测试（16 项，含 4 项心跳探活）
- ✅ AI 工具层错误路径测试（13 项，含 6 项新补）
- ✅ Telnet 传输层边界测试（4 项）
- ✅ 客户端 WebSocket 客户端测试（11 项）
- ✅ 客户端状态 store 测试（17 项）
- ✅ 客户端 RPC 函数测试（8 项）
- ✅ 构建验证通过（`pnpm build`）

**当前状态：** 161 项单元测试全绿，覆盖率 84%/68%/84%/88%

**待完成：**
- ✅ 修 Telnet IAC 协商 bug（跨 chunk 拼接已修复，自相矛盾已修复）
- ✅ 补 ws-io.ts 心跳探活测试（4 项：30s ping / 断开 clearInterval / 非 loopback 拒绝 / disposer 清理）
- ⏳ 手工验收 10 个场景（第三块）
- ⏳ 手工验收补充 6 个遗漏场景（见文末）
...
**通过标准**：`pnpm test` 输出 `161 passed (161)`，没有红色报错。
...
**操作**：新建 SSH 连接 → 输入密码 → 点密码框旁的眼睛图标

**通过**：密码在明文/掩码之间切换；切换后表单值不丢失

---

#### Telnet 模式切换（Raw vs Telnet）

**操作**：新建 Telnet 连接 → 展开高级选项 → 切换「Raw TCP」/「Telnet」→ 保存 → 连接

**通过**：两种模式都能连上模拟设备；Telnet 模式下终端输出无乱码

---

#### 刷新页面后状态恢复

**操作**：已连接一个会话 → 按 F5 刷新页面 → 重新打开终端管理面板

**通过**：之前保存的连接配置还在；已连接的会话可能需要重新连接（这是预期行为，不算失败）

---

#### 快速连点连接/断开

**操作**：对一个连接卡片快速连点 5 次连接按钮

**通过**：只创建一个会话（不会重复创建）；界面不卡死

---

#### SSH 密钥认证连接

**操作**：准备一个密钥对 → 新建 SSH 连接 → 认证方式选「密钥」→ 粘贴私钥 → 连接
...
| 顺序 | 做什么 | 谁做 | 状态 |
|---|---|---|---|
| 1 | 修 Telnet IAC 的 Bug | 给 AI | ✅ 已修复（跨 chunk 拼接） |
| 2 | 补入口装配测试 | 给 AI | ✅ 已完成（10 项） |
| 3 | 补 HTTP 路由层测试 | 给 AI | ✅ 已完成（8 项） |
| 4 | 补 WS 生命周期测试 | 给 AI | ✅ 已完成（16 项，含 4 项心跳探活） |
| 5 | 补工具层错误路径测试 | 给 AI | ✅ 已完成（13 项，含 6 项新补） |
| 6 | 补 Telnet 边界测试 | 给 AI | ✅ 已完成（4 项） |
| 7 | 补客户端 ws.ts 测试 | 给 AI | ✅ 已完成（11 项） |
| 8 | 补客户端 store.ts 测试 | 给 AI | ✅ 已完成（17 项） |
| 9 | 补客户端 rpc.ts 测试 | 给 AI | ✅ 已完成（8 项） |
| 10 | 构建验证 | `pnpm build` | ✅ 已通过 |
| 11 | 手工验收 10 个场景 | 你自己点 | ⏳ 待做 |
| 12 | 手工验收补充 6 个场景 | 你自己点 | ⏳ 待做 |
