...
.tm-overlay { position:fixed; top:0; bottom:0; z-index:50; display:flex; justify-content:flex-end; background:var(--dsw-alias-bg-base, #fff); color:var(--dsw-alias-label-primary, #000); font:14px/1.5 var(--dsw-font-family, system-ui, sans-serif); box-shadow:-2px 0 12px rgba(0,0,0,.12); }
...
/* 终端列封顶 760px + 贴右：终端与连接面板始终相连（无中间留白），封顶后多出的空间作为左侧缓冲，终端不再挤着聊天；小屏不触发封顶、照常填满 */
.tm-main { flex:1 1 auto; min-width:0; max-width:760px; display:flex; flex-direction:column; border-right:1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.1)); }
