...
| 灵枢平台 MaaS 绑定 | 1.1 第 3 条「初版 MaaS 服务绑定灵枢平台，API 简化配置」具体指什么？（DSH 的模型服务配置指向灵枢？还是插件要调灵枢的 API？） | 影响配置模块与文档 | **待用户解释**，解释前不动工 |
| AgentTerm 参考 | 需要 AgentTerm 的链接/截图/安装包，研究其 SSH/Telnet 连接设计（用户使用习惯） | M4 连接表单与交互细节 | **待用户提供**；拿到后在 M4 开工前对照调整 |
